...
{% macro databricks__get_merge_sql(target, source, unique_key, dest_columns, incremental_predicates) %}
  ...
  {%- set matched_update_condition = config.get('matched_update_condition') -%}
  {%- set matched_delete_condition = config.get('matched_delete_condition') -%}
  ...
  {% endif %}
    ...
    {%- if not skip_matched_step and matched_delete_condition%}
    when matched and ({{ matched_delete_condition }}) then delete
    {%- endif %}
